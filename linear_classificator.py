...
from loader import load_no_show_issue, filter_features
import itertools

x_appointment, y_appointment, x_names, y_names = load_no_show_issue()
features = ['Age', 'Gender', 'Registrati', 'DayOfTheWe', 'DayOfTheYe', 'Diabetes', 'Alcoolism',
            'HiperTensi', 'Handcap', 'Smokes', 'Scholarshi', 'Tuberculos', 'Sms_Remind',
            'AwaitingTi']
...
print('max{} features:{}'.format(m, res[m]))
...
#
# kfold5 = KFold(x_appointment.shape[0], 5, shuffle=True)
# valores = cross_val_score(modelo, x_appointment, y_appointment, cv=kfold5)
